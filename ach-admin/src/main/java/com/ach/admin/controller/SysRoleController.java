...
public class SysRoleController extends BaseController {
    ...
    @GetMapping("/list")
    public BaseResponseData<PageCustomDTO<RoleDTO>> list(RoleQuery query) {
    ...
    public void export(HttpServletResponse response, RoleQuery query) {
        ...
        List<RoleDTO> records = page.getRecords().stream().map(RoleDTO::new).collect(Collectors.toList());
        PageCustomDTO<RoleDTO> pageCustomDTO = new PageCustomDTO<>(records, page.getTotal());
        CustomExcelUtil.writeToResponse(pageCustomDTO.getRows(), RoleDTO.class, response);
    }
    ...
    public BaseResponseData<PageCustomDTO<UserDTO>> allocatedUserList(@PathVariable("roleId") Long roleId,
                                                                      AllocatedRoleQuery query) {
        query.setRoleId(roleId);
        PageCustomDTO<UserDTO> page = sysRoleService.getAllocatedUserList(query);
    ...
    public BaseResponseData<PageCustomDTO<UserDTO>> unallocatedUserList(@PathVariable("roleId") Long roleId,
                                                                        UnallocatedRoleQuery query) {
        query.setRoleId(roleId);
        PageCustomDTO<UserDTO> page = sysRoleService.getUnallocatedUserList(query);
        return BaseResponseData.ok(page);
    }
    /**
     * 批量取消授权用户
